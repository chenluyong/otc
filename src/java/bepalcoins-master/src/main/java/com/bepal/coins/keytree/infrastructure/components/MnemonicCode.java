...
import com.bepal.coins.crypto.PBKDF2SHA512;
...
//    }
     ...
     */
    public MnemonicCode(InputStream wordstream, String wordListDigest) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader(wordstream, "UTF-8"));
        this.wordList = new ArrayList<>(2048);
        MessageDigest md = SHAHash.newDigest2256();
        String word;
...
//        }
     ...
     */
    public byte[] toEntropy(List<String> words) throws Exception {
        if (words.size() % 3 > 0) {
        ...
        byte[] hash = SHAHash.Sha2256(entropy);
     ...
     */
    public List<String> toMnemonic(byte[] entropy) throws Exception {
        if (entropy.length % 4 > 0) {
            throw new Exception("Entropy length not multiple of 32 bits.");
        }

        if (entropy.length == 0) {
            throw new Exception("Entropy is empty.");
        }
     ...
     */
    public void check(List<String> words) throws Exception {
        toEntropy(words);
    }
