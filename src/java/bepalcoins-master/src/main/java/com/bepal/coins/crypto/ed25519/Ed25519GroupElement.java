package com.bepal.coins.crypto.ed25519;
...
public class Ed25519GroupElement implements Serializable {
    ...
    public Ed25519GroupElement addToPub(final Ed25519GroupElement g) {
        if (this.coordinateSystem != CoordinateSystem.P3) {
            throw new UnsupportedOperationException();
        }
...
//        final Ed25519FieldElement D;
        ...
        YPlusX = this.Y.add(this.X);
        YMinusX = this.Y.subtract(this.X);
        Ed25519FieldElement td2 = new Ed25519EncodedFieldElement(Hex.decode("59f1b226949bd6eb56b183829a14e00030d1f3eef2808e19e7fcdf56dcd90624")).decode();
        Ed25519FieldElement Z = this.Z;
        Ed25519FieldElement T2d = this.T.multiply(td2);

        Ed25519FieldElement RX = g.Y.add(g.X);
        Ed25519FieldElement RY = g.Y.subtract(g.X);
        Ed25519FieldElement RZ = RX.multiply(YPlusX);
        RY = RY.multiply(YMinusX);
        Ed25519FieldElement RT = T2d.multiply(g.T);
        RX = g.Z.multiply(Z);
        Ed25519FieldElement t0 = RX.add(RX);
        RX = RZ.subtract(RY);
        RY = RZ.add(RY);
        RZ = t0.add(RT);
        RT = t0.subtract(RT);
        return p1xp1(RX, RY, RZ, RT);
    }
    ...
    private Ed25519GroupElement select(final int pos, final int b) {
        // Is r_i negative?
        final int bNegative = ByteArrayData.isNegativeConstantTime(b);
        // |r_i|
        final int bAbs = b - (((-bNegative) & b) << 1);

        // 16^i |r_i| B
        final Ed25519GroupElement t = Ed25519Group.ZERO_PRECOMPUTED
                .cmov(this.precomputedForSingle[pos][0], ByteArrayData.isEqualConstantTime(bAbs, 1))
                .cmov(this.precomputedForSingle[pos][1], ByteArrayData.isEqualConstantTime(bAbs, 2))
                .cmov(this.precomputedForSingle[pos][2], ByteArrayData.isEqualConstantTime(bAbs, 3))
                .cmov(this.precomputedForSingle[pos][3], ByteArrayData.isEqualConstantTime(bAbs, 4))
                .cmov(this.precomputedForSingle[pos][4], ByteArrayData.isEqualConstantTime(bAbs, 5))
                .cmov(this.precomputedForSingle[pos][5], ByteArrayData.isEqualConstantTime(bAbs, 6))
                .cmov(this.precomputedForSingle[pos][6], ByteArrayData.isEqualConstantTime(bAbs, 7))
                .cmov(this.precomputedForSingle[pos][7], ByteArrayData.isEqualConstantTime(bAbs, 8));
        // -16^i |r_i| B
        //noinspection SuspiciousNameCombination
